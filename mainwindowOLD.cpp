...
SerialBb.Read(comStringIn, 100, &nBytesIn);*/
    ...
    sem_init (&RunScan_count, 0, 0);// Remove this and all associated
...
//        if( strcmp(FileExtension, ".bsqmn") == 0 ) {
//            FileExtension[0] = 0;
//        }*/
...
#define BASEVALUE 5000
            for(unsigned i=1 ; i<AcqImgSize ; i++) {
                ...
                if(MaxVal<(int)BASEVALUE+(int)m_nPhotonFrameBuffer[i]-(int)m_nPhotonBackgroundBuffer[i])
                    MaxVal=(int)BASEVALUE+(int)m_nPhotonFrameBuffer[i]-(int)m_nPhotonBackgroundBuffer[i];
                //if((i!=6385)&&(i!=10266)&&(i!=10587)&&(i!=28967)&&(i!=31323)&&(i!=50388)&& (MinVal>m_nPhotonFrameBuffer[i]) && (m_nPhotonFrameBuffer[i]>300))
                if( (MinVal>(int)BASEVALUE+(int)m_nPhotonFrameBuffer[i]-(int)m_nPhotonBackgroundBuffer[i]) /*&& (m_nPhotonFrameBuffer[i]>100)*/ )
                    MinVal=(int)BASEVALUE+(int)m_nPhotonFrameBuffer[i]-(int)m_nPhotonBackgroundBuffer[i];
            }
            if(MaxVal == MinVal) {
                MaxVal = MinVal+1;
            }
...
#define lala
            ...
            for(int i=0 ; i<DisplayHeight ; i++) {
                ...
                for(int j=0 ; j<DISPLAYWIDTH ; j++) {
                    if(ExitProgram) break;
                    long MyImgData8 = (uchar)( 255.0 * (float)((int)BASEVALUE + (int)*Input - (int)*Background - MinVal) / (float)(MaxVal-MinVal+1) + .5);
                    ImgData8 = (uchar)( 255.0 * (float)((int)BASEVALUE + (int)*Input - (int)*Background - MinVal) / (float)(MaxVal-MinVal+1) + .5);
...
BOOL m_bUseCMigit = TRUE;
            if (m_bUseCMigit) {
                // *** Make GPS Filename
                strcpy(GPSFilePath, FilePath);
                GPSFilePath[strlen(GPSFilePath)-3] = 0;
                strcat(GPSFilePath, ".gps");
