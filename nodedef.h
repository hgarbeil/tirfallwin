...
//! Reaction Wheel structure: z axis is aligned with axis of RotationA.
typedef struct
{
	//! Component Type
	uint16_t type;
	...
	//! Device information time stamp
	double utc;
	//! Rotates vectors from RW frame (axis of RotationA = z) to body frame.
	quaternion align;
...
typedef struct
{
	...
	//! Connection information for device.
	uint16_t portidx;
	//! Requested Magnetic Field strength.
	float rfld;
	//! Actual Magentic Field Strength.
	float fld;
} mtrstruc;
...
typedef struct
{
	...
	float flw;
	//! RotationA of thrust vector (+z) in to node frame.
	float isp;
} thststruc;
...
//! Device structure
/*! Complete details of each component. It is a union of all the
possible component types, with a generic type for looking up basic
information.
*/
typedef struct
{
