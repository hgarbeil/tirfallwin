    ...
    ui(new Ui::ProcessResultsDlg)
{
    ...
    ui->radiancePlot->setRescaleFlag(true);
    ui->emissivityPlot->setRescaleFlag(true);
...
void ProcessResultsDlg::plotWavecal (int type) {
    ...
    int i ;
    float *xdata = new float [200] ;
    float *ydata = new float [200] ;
    for (i=0; i<200; i++) xdata[i] = i ;
    switch (type) {
        // plot pixel vs wavelength in microns
        case 0 :
        for (i=0; i<200; i++) ydata[i] =gp->wavelen[i];
        ui->wavePlotWidget->setXYData (0, xdata, gp->wavelen,200) ;
        ...
        ui->wavePlotWidget->xAxis->setRange (0,200) ;
        ui->wavePlotWidget->replot() ;

        break ;

        case 1:
        for (i=0; i<200; i++) ydata[i] =10000./gp->wavelen[i];
        ui->wavePlotWidget->setXYData (0, xdata, ydata, 200) ;
...
void ProcessResultsDlg::loadStackedImage (int colNum){
    ...
    for (i=0; i<nl; i++){
        for (j=0; j<ns; j++){
            imdat[i * ns + j] = so->outarr[i * ns_inst * ns + colNum * ns + j] ;
        }
    }
...
void ProcessResultsDlg::loadBands(){
    ...
    for (ib=gp->stopFinWave; ib>=gp->startFinWave; ib--){
...
void ProcessResultsDlg::radClicked (QPoint qpt){
    ...
    //for (ib=gp->startFinWave; ib<=gp->stopFinWave; ib++){
    for (ib=0; ib<nwaves; ib++){
        sampnum = gp->stopFinWave - ib ;
        yvals [ib] = gp->radArray [yloc * gp->nsamps * nwaves +  xloc * nwaves + ib] ;
        if (emissFlag)
            yyvals [ib] = gp->emissArr [yloc * gp->nsamps * nwaves +  xloc * nwaves + ib] ;
        xvals [ib] = gp->wavelen[sampnum] ;
     }
...
void ProcessResultsDlg::emissClicked (QPoint qpt){
    ...
    for (ib=0; ib<nwaves; ib++) {
        sampnum = gp->stopFinWave - ib ;
        yvals [ib] = dataArr [yloc * gp->nsamps * nwaves +  xloc * nwaves + ib] ;
        xvals [ib] = gp->wavelen[sampnum] ;
     }
...
void ProcessResultsDlg::on_closeButton_clicked()
{
    emit (closeUp()) ;
}
