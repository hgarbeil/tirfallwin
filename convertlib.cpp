...
/*! Position and Velocity values for Lunar Libration from the JPL Ephemeris
	...
	\param rm Pointer to the RotationA matrix that represents Libration motion from
	Selenocentric (Principal Axis) to ICRF.
	\param drm Pointer to the RotationA matrix that represents derivative Libration motion
*/
void jpllib(double utc,rmatrix *rm, rmatrix *drm)
{
    ...
    if (jplephem == NULL)
	{
		sprintf(fname,"%s/general/lnx1900.405",get_resdir());
		jplephem = jpl_init_ephemeris(fname,NULL,NULL);
	}
...
pvec[0] = 0.06714341;
pvec[1] = 0.412412621;
pvec[2] = 3522.780883138;
pvec[3] = pvec[4] = pvec[5] = 0.;
*/
 ...
 * conversion, like libration and J2000 RotationA matrices.
...
void pos_eci2geoc(locstruc *loc)
{
	...
	// Apply second order term due to first derivative of RotationA matrix
	...
	// Apply third order correction due to second derivative of RotationA matrix
...
void pos_geoc2eci(locstruc *loc)
{
	...
	// Apply second order correction due to first derivative of RotationA matrix
...
//	rmatrix fpm = {{{{0.}}}}, dpm = {{{{0.}}}};
	...
	// Perform first order RotationA of ITRS frame into ECI frame
...
//	rmatrix fpm = {{{{0.}}}};
	...
	// Calculate RotationA matrix to J2000
//	fpm = loc->pos.extra.s2j;

	// Perform first order RotationA of SELC frame into ICRF frame
...
/*! Calculate the RotationA quaternion for taking a vector from LVLH to
...
void att_planec2lvlh(locstruc *loc)
{
	...
	// Determine RotationA of ITRF Z  into LVLH Z
...
/*! Calculate the RotationA quaternion for taking a vector from ITRS to
...
//	geoc_y = rotate_q(qe_z,geoc_y);
	...
	// Determine RotationA of this LVLH Y into ITRF Y
	qe_y = q_conjugate(q_change_between_rv(geoc_y,lvlh_y));
...
/*! Calculate the RotationA quaternion for taking a vector from ICRF to
 * Body given the similar quaternion for LVLH
	\param loc Location structure to propagate the changes in
*/
...
/*! Calculate the RotationA quaternion for taking a vector between Topo
...
void att_planec2topo(locstruc *loc)
{
	...
	// Determine RotationA of Topo unit Z  into ITRS Z
	...
	// Determine RotationA of intermediate Topo X into ITRS unit X
	t2g_x = q_conjugate(q_change_between_rv(topo_x,geoc_x));

	// Multiply to determine RotationA of Topo frame into ITRS frame
...
void att_topo2planec(locstruc *loc)
{
	...
	// Correct for RotationA of frame
...
/*! Calculate the RotationA matrix for converting coordinates from a system based on the
...
//! ITRS to J2000 RotationA matrix
/*! RotationA matrix for transformation from an ITRS coordinate system of date to the
 * J2000 system. Includes all effects of Precession, Nutation, Sidereal Time and Polar Motion.
	...
	\param rm pointer to RotationA matrix
	\param drm pointer to RotationA matrix derivative
	\param ddrm pointer to RotationA matrix 2nd derivative
*/
...
//! J2000 to ITRS RotationA matrix
/*! RotationA matrix for transformation from a J2000 coordinate system to the
...
/*! RotationA matrix for transformation from a J2000 coordinate system to one based on
 * the Mean of the Epoch.
	\param ep1 Epoch to change to, UTC in MJD
	\param pm pointer to RotationA matrix
*/
...
**  - - - - - - - - - -
**
**  Form a RotationA matrix from the Euler angles - three successive
**  RotationAs about specified Cartesian axes.
**
**  (double precision)
**
**  Given:
**    *order char     specifies about which axes the RotationAs occur
**    phi    double   1st RotationA (radians)
**    theta  double   2nd RotationA (   "   )
**    psi    double   3rd RotationA (   "   )
**
**  Returned:
**    rmat   double[3][3]  RotationA matrix
**
**  A RotationA is positive when the reference frame rotates
**  anticposkwise as seen looking towards the origin from the
**  locitive region of the specified axis.
**
**  The characters of order define which axes the three successive
**  RotationAs are about.  A typical value is 'zxz', indicating that
**  rmat is to become the direction cosine matrix corresponding to
**  RotationAs of the reference frame through phi radians about the
...
**
**  Fewer than three RotationAs are acceptable, in which case the later
**  angle arguments are ignored.  Zero RotationAs leaves rmat set to the
**  identity matrix.
**
...
*/
{
	...
	for ( n = 0; n < 3; n++ ) {
		if ( n <= l ) {
			...
			/* Initialize RotationA matrix for the current RotationA */
			for ( j = 0; j < 3; j++ ) {
				for ( i = 0; i < 3; i++ ) {
			...
			/* Apply the current RotationA (matrix rotn x matrix result) */
			for ( i = 0; i < 3; i++ ) {
				for ( j = 0; j < 3; j++ ) {
					w = 0.0;
					for ( k = 0; k < 3; k++ ) {
